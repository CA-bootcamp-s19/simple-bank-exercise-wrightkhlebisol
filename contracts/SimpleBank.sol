...
contract SimpleBank {
    ...
    // Modifiers
    // Ensure sender is enrolled in bank
    modifier depositModifier(){
        require(enrolled[msg.sender], "You are not enrolled in the bank yet");
        _;
    }

    // Ensure the amount to withdraw is more than or equal the amount in bank
    modifier withdrawModifier(uint withdrawAmount){
    ...
    function getBalance() public view depositModifier returns (uint) {
    ...
    function deposit() public payable depositModifier returns (uint) {
        /* Add the amount to the user's balance, call the event associated with a deposit,
          then return the balance of the user */
          balances[msg.sender] += msg.value;
          emit LogDepositMade(msg.sender, msg.value);
          emit LogBalance(msg.sender, balances[msg.sender], msg.value);
          return balances[msg.sender];
    }
    ...
    // Emit the appropriate event
    function withdraw(uint withdrawAmount) public depositModifier returns (uint) {
